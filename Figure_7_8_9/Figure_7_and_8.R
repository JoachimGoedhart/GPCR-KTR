...
##### READ THE DATA AND RENUMBER THE CLUSTERS ########
  ...
  df_Erk <- cluster1 %>% dplyr::select(cond_inh,Unique_Unique_Object,Time_in_min,ERK,Cluster=clu_E)

  df_Akt <- cluster1 %>% dplyr::select(cond_inh,Unique_Unique_Object,Time_in_min,Akt,Cluster=clu_A)
  ...
  df_contribution <- cluster1 %>% dplyr::select (cond_inh, clu_E, clu_A, Unique_Unique_Object) %>% distinct()
  ...
  png(file=paste0("Figure_7.png"), width = 4000, height = 3600, units = "px", res = 400)
  (p_cERK/p_ERK) +   
  ...
  png(file=paste0("Figure_8.png"), width = 4000, height = 2800, units = "px", res = 400)
  (p_cAkt/p_Akt) +   
    # plot_layout(design = layout) +
 ...
 bubble_plot <-  ggplot(data=df_contribution_subset, aes(x=clu_E,y=clu_A))+geom_count()+facet_wrap(~cond_inh) +
  ...
  png(file=paste0("Figure_8_new.png"), width = 4000, height = 4400, units = "px", res = 400)
  (p_cAkt/p_Akt/bubble_plot) +   
    # plot_layout(design = layout) +
    plot_layout(heights = c(1, 2,1.6))+
    NULL
